...
│       ├── ChipDot (亮黄小圆点 22×22)
│       └── SchoolText (学校名+排名)
├── ContentCard (60,470,960×877,白色悬浮卡片)  ← z 在 Hero 之上;高度固定,不随文字长短变化
│   ├── DearRow (Dear + 马赛克 + ,)
│   ├── Line2 (前文,可选)
│   ├── HighlightBox (红框关键句)
│   ├── Line3 (后文,可选)
...
| 属性 | 值 |
|---|---|
| 位置/尺寸 | x=**60**, y=**470**, 960 × **877** |
| **高度必须写死 877** | ⚠️ 卡片高度 **固定 = 877**,**绝对不要用 `hug_contents`**。文字少的海报也必须是 877,下半部分自然留白(以「李晨瑞-MSU」这张满高卡片为基准)。固定后所有海报卡片底边都在 y=**1347**,与 slogan(y=1382)间距恒定。 |
| 圆角 | **32** |
| 填充 | 白色 #FFFFFF |
