...
2. Inspect and transcribe only visible screenshot content. Use the filename only as a hint and output identifier.
   - **One screenshot = one poster is the DEFAULT. Never merge on your own initiative.** The user's standing preference (stated 2026-09-16): every screenshot in the inbox gets its own poster (`Xxx University.png` + `Xxx University2.png` → two posters, output names identical to the input names). Even a first message inviting a meeting plus a later message confirming the time/room should stay as two separate posters unless the user says otherwise.
   - **Only exception: byte-identical duplicates.** If two inbox files have the same MD5 (e.g. `蒋元萍University of Macau.png` + `蒋元萍University of Macau 2.png`), produce ONE poster and report the duplicate. Verify with a hash, not by eyeballing.
   - If you do think two screenshots are really one story, **ask the user first** — do not silently merge. Then in the completion report always state `N inputs → M posters` and list exactly which inputs were absorbed and why.
...
5. Identify the school from screenshot evidence. Verify the latest public ranking immediately before generation:
   - **United States ONLY: use the latest U.S. News Best National Universities ranking (美国国内综合大学排名), NOT the Best Global Universities list.** Label `学校中文名｜USNEWS数字`. (User-decided rule, 2026-09-21.) So a U.S. school may show a mid-table number here even though its world rank is 400+. Do not "correct" these numbers back to the global list. 2026 edition reference values: 雪城大学｜USNEWS75, 布法罗大学｜USNEWS75, 卡内基梅隆大学｜USNEWS20, 宾夕法尼亚大学｜USNEWS7. The edition refreshes every September — always re-check which edition is current.
...
6. Generate a branded poster on the Ardot canvas following the layout, color, type, watermark, and mosaic rules in [references/template-rules.md](references/template-rules.md). Use the type/color mapping (green/purple/blue) from the classification. Then export the poster frame as PNG.
   - **PDF admission letters (Offer) — never paraphrase, never translate.** Keep the blue Offer template (`博士OFFER来啦!`), but do NOT retype the letter as poster text. Put a **screenshot of page 1** inside the ContentCard instead:
     1. Render page 1 with `pymupdf` at 300 dpi; crop away the outer blank margins (the letterhead logo can be kept as a strip above the body — it makes the offer read as authentic).
     2. Mosaic any personal data. Offer letters often have **no salutation and no name/ID on page 1** — if there is genuinely nothing personal, do NOT invent redactions, just say so in the report. Always mask literal URLs in the body (the standing "links get masked" rule).
     3. Upload it (`register_assets` → `curl -X PUT '<uploadUrl>'` → `upload_images` with the paired `downloadUrl`) onto a frame inside the ContentCard.
     4. Sizing: the card's inner area is 840×774, so a portrait page fits at ≈592×774 (height-bound). Center it by setting `counterAxisAlignItems: "CENTER"` and `primaryAxisAlignItems: "CENTER"` on the ContentCard.
     5. Save the poster as the original PDF name with a `.png` extension; the PDF itself is kept untouched.
